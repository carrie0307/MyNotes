...
def mongo_insert(collection, mydict):
    ...
    _id = collection.insert(mydict)
    # _id就是刚刚插入数据的ObjectID
...
def mongo_query(db, collection,mydict):
    ...
    '''另一个例子'''
    db.term.aggregate([
     {'$match':{'library_id':3607}}, # 选择library_id为3607的文档
     {'$limit':5},                 # 只读取5条数据
...
def ref_operate(collection):
    ...
    {
    "_id" : ObjectId("5a3ccaaa917399ce94ba814e"),
    "address" : {
        "$ref" : "address",
        "$id" : ObjectId("5a3cca1c917399ce94ba813a"),
        "$db" : "ip_whois"
    },
    "contact" : "987654321",
    "dob" : "01-01-1991",
    ...
    '''
    res = collection.find_one({"name": "Tom Benzamin"})
    '''res['address'] 是DBRef类型，其文档见https://api.mongodb.com/python/1.0/pymongo.dbref.DBRef-class.html'''
    print res['address'].id # 即得到所引用文档的id
    collection = db['address']
    print collection.find_one({"_id": res['address'].id}) # 即可得到所引用文档的内容
    
    '''先将引用部分空下来，待被应用数据库有数值后，再插入具体的ID等'''
    collection = db['test']
    collection.insert({'ip':'1.1.1.1','info':{}})
    collection2 = db['address']
    _id = collection2.insert({'Province':'Shandong','Region':'Weihai'})
    '''from bson.dbref import DBRef  # 注意DBRef'''
    from bson.dbref import DBRef
    collection.update({'ip': '1.1.1.1'}, {"$set":{"info":DBRef('address',_id,'ip_whois')}})

    res = collection.find_one({"ip": "1.1.1.1"})
    collection2 = db['address']
    print collection2.find_one({"_id": res['info'].id}) # 即可得到所引用文档的内容


def lookup_operate(collection):
    '''
    # https://piaosanlang.gitbooks.io/mongodb/content/04day/section4.5.html
