...
3. 根据每个词出现次数，构建面向整个文件的特征词集合：feature_words (选择了出现次数最多TOP1000词)
4. 根据所有feature_words，计算训练集、测试集每个文件的向量表示[用0,1表示词袋模型中的词是否在某个文件中出现]；
5. 使用sklearn中的MultinomialNB多项式贝叶斯进行分类
